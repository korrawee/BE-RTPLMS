...
import { RequestDto } from '../../relations/request/dto/Request.dto';
...
export class OtRequestService {
    ...
    public async getOtRequest(accId: string) {
        ...
        const requestOfAccount: RequestDto[] =
            await this.requestService.getRequestByAccountId(accId);

        const requestWithWorkTime = await Promise.all(
            requestOfAccount.map(async (reqObj: RequestDto) => {
    ...
    private async getWorkTime(reqObj: RequestDto) {
        const shiftTimeObj: { shift_time: string } =
            await this.shiftService.getShiftTimeById(reqObj.shift_code);

        // end time of shift = start time of OT then start time of OT = shift's start time plus 8
        const startTime = moment(shiftTimeObj.shift_time, 'HH:mm:ss')
            .add(8, 'hours')
            .format('hh:mm');
